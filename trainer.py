...
#from degree_weighted_sampling import node_degree, negative_sampling
#from degree_weighted_sampling_nosequential import node_degree, negative_sampling
...
class Trainer:
    ...
    def eval_epoch(self, epoch, valid_batch_indices, N_steps, N_negs) -> Dict:
        ...
        for i, batch_dict in enumerate(self.valid_loader):
            ...
            #degree weighted & degree proportional 
            #sampled_negative_triplets = negative_sampling(self.model, hr_vector[:self.args.walks_num//2], self.valid_pathes_dict, self.valid_pathes_dist, self.valid_deg_dist, self.valid_neighbors_degree,
            #                                              N_steps, N_negs, self.valid_graph, self.valid_directed_graph, head_ids)
            ...
            criterion = nn.CrossEntropyLoss(weight = log_tail_deg).cuda()
            # head + relation -> tail
        ...
        '''
        #start to pick next batch index
        unseen = []
        valid_batch_indices = []
        for t,n in self.valid_count_check.items():
            if n==0:
                unseen.append(t)
        print(f'{epoch}epoch valid unseen: {len(unseen)}')  
        if len(unseen) != 0:
            unseen_batch = defaultdict()       
            for u in unseen:
                ids = self.valid_triplet_mapping[u]
                for id in ids:
                    if id not in unseen_batch:
                        unseen_batch[id] = 1
                    else:
                        unseen_batch[id] +=1
            top_k_items = heapq.nlargest(5, unseen_batch, key=unseen_batch.get) 
            for idx in top_k_items:
                self.valid_batch_appear[idx] +=1
            valid_batch_indices = list(top_k_items)
        
        if len(valid_batch_indices) < self.valid_steps:
            valid_count = sorted(self.valid_batch_count, key=self.valid_batch_count.get)
            for item in valid_count:
                if item not in valid_batch_indices:
                    valid_batch_indices.append(item)
                    self.valid_batch_appear[item] +=1
                if len(valid_batch_indices) == self.valid_steps:
                    break
    ...
    def train_epoch(self, epoch, batch_indices, N_steps, N_negs):
        ...
        for i, batch_dict in enumerate(self.train_loader):
            ...
            #degree weighted & degree proportional 
            #sampled_negative_triplets = negative_sampling(self.model, hr_vector[:self.args.walks_num//2], self.train_pathes_dict, self.train_pathes_dist, self.train_deg_dist, self.train_neighbors_degree,
            #                                            N_steps, N_negs, self.graph, self.directed_graph, head_ids)
            
            #normal dist & uniform dist (original mcmc) 
        ...
        unseen = []
        batch_indices = []
        for t,n in self.train_count_check.items():
            if n==0:
                unseen.append(t)
        print(f'{epoch}epoch num_unseen: {len(unseen)}')  
        if len(unseen) != 0:
            unseen_batch = defaultdict()
            for u in unseen:
                ids = self.train_triplet_mapping[u]
                for id in ids:
                    if id not in unseen_batch:
                        unseen_batch[id] = 1
                    else:
            ...
            top_k_items = heapq.nlargest(num, unseen_batch, key=unseen_batch.get) 
            for idx in top_k_items:
                self.train_batch_appear[idx] +=1
            batch_indices = list(top_k_items)   
        
        count = sorted(self.train_batch_count, key=self.train_batch_count.get)
        for item in count:
            if item not in batch_indices:
                batch_indices.append(item)
                self.train_batch_appear[item] +=1
            if len(batch_indices) == self.training_steps:
                break
        ''' 
