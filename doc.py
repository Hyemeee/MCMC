...
def load_data(path: str, graph, directed_graph, walks_num):
    ...
    logger.info('time to make dfs pathes {}'.format(datetime.timedelta(seconds=(end-start))))
    logger.info('Load {} dfs pathes'.format(num_dfs_path))     
    logger.info('start to make dfs pathes file')
    with open('data/wiki5m_trans/dfs_pathes.json', 'w', encoding='utf-8') as f:
